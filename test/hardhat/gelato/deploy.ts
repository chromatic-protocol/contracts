...
async function deployTaskTreasury(
  gelato: string
): Promise<TaskTreasuryUpgradable> {
...
export async function deploy() {
  ...
  const timeModule = await (
    await ethers.getContractFactory("TimeModule")
  ).deploy()
  ...
  const singleExecModule = await (
    await ethers.getContractFactory("SingleExecModule")
  ).deploy()
...
export async function resolverTestDeploy() {
  ...
  const { address: testCountResolverAddress } = await deployments.deploy(
    "CountTaskResolver",
    {
      from: deployer,
      args: [],
  ...
  const { address: testCountTaskAddress } = await deployments.deploy(
    "IncreaseCountTaskTest",
    {
      from: deployer,
      args: [
  ...
  const testCountTask = await ethers.getContractAt(
    "IncreaseCountTaskTest",
    testCountTaskAddress
  )
