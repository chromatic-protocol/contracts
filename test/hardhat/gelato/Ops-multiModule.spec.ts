...
async function fixture() {
  return await hardhatErrorPrettyPrint(async () => {
    ...
    const counter = await (
      await ethers.getContractFactory("CounterWL")
    ).deploy()
    ...
    await fETH
      .connect(user)
      .approve(taskTreasury.address, ethers.constants.MaxUint256)

    // deposit funds
    const depositAmount = ethers.utils.parseEther("10")
    await taskTreasury
      .connect(user)
      .depositFunds(user.address, fETH.address, depositAmount)
...
describe("Automate multi module test", function () {
  ...
  beforeEach(async function () {
    ...
    await automate
      .connect(user)
      .createTask(counter.address, execSelector, moduleData, fETH.address)
  ...
  it("getTaskId", async () => {
    ...
    ](userAddress, counter.address, execSelector, moduleData, fETH.address)
  ...
  it("no modules", async () => {
    ...
    await automate
      .connect(user)
      .createTask(counter.address, execData, moduleData, fETH.address)
