...
export const getTokenFromFaucet = async (
  ...
  const faucetEthBalance = await (await ethers.provider.getSigner(faucet)).getBalance()
  ...
  if (
    ...
    if (ethSignerBalance.lt(oneEth)) throw Error(`ETH Faucet has insufficient: ${tokenAddress}`)
  ...
  if (signerBalance.lt(amount)) throw Error(`Faucet has insufficient: ${tokenAddress}`)
  ...
  if (recepientBalance.lt(amount)) throw Error(`Tranfer not succesfull: ${tokenAddress}`)
}
